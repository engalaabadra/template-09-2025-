...
use App\Repositories\User\LikeRepository;
...
class LikeController extends BaseController
{
    ...
    public function storeLikeContent(Content $content)//addLikeContent
    {
        $result = $this->likeService->storeLikeContent($this->like, $content);

        // For API requests, respond with data wrapped in LikeResource
        return $this->respond($result, LikeResource::class);
    }

    public function storeLikeComment(Comment $comment)//addLikeComment
    {
        $result = $this->likeService->storeLikeComment($this->like, $comment);

        // For API requests, respond with data wrapped in LikeResource
        return $this->respond($result, LikeResource::class);
    }

    public function storeLikeReply(Reply $reply)//addLikeReply
    {
        $result = $this->likeService->storeLikeReply($this->like, $reply);
